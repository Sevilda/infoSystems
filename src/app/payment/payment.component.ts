...
export class PaymentComponent implements OnInit {
  ...
  async addPayment(){
    if (this.paymentForm.valid){
      ...
      console.log(payment);
      if (confirm("Do you want to add a payment of " + payment.amount + "$,  to the tenant " + payment.tenant.name + " ?")) 
      { await this.paymentService.addPayment(payment);
        await this.tenantService.updateBalance(payment.tenant);
        this.success="Added payment of " + payment.amount + "$ to tenant " + payment.tenant.name;
        this.error=""
